...
### 오디오 포커스
- ExoPlayer 내장 오디오 포커스 사용 중
- 수동 AudioFocusListener 추가 금지
- 설정 제거 시 전화 중에도 재생되는 문제 발생

### Gap 중 일시정지 → 재개
- 남은 Gap 시간부터 재개해야 함 (처음부터 X)
- Gap 상태 저장/복원 로직 수정 시 실기기에서 LR 모드 테스트

### 녹음 일시정지/재개
- API 24+ 전용 기능
- LRLR 모드에서 녹음 중 pause → resume 시 녹음 이어져야 함

### 앱 종료 시 진행 저장
- 스와이프 종료해도 현재 청크 위치 저장되어야 함
- 재시작 시 저장된 위치에서 재개
